...
class SemanticPOSS(data.Dataset):
    def __init__(self, config, dataset_type='train', flip_aug=False):
        ...
        with open(self.label_mapping, 'r') as stream:
            sempossyaml = yaml.safe_load(stream)
        self.mapped_class_name = sempossyaml['mapped_class_name']
        self.learning_map = sempossyaml['learning_map']
        if self.dataset_type == 'train':
            split = sempossyaml['split']['train']
        elif self.dataset_type == 'valid':
            split = sempossyaml['split']['valid']
        elif self.dataset_type == 'test':
            split = sempossyaml['split']['test']
        else:
        ...
        self.class_names = [ "empty", "people", "rider", "car",  
                             "trunk", "plants", "traffic-sign", "pole",
                             "building", "fence", "bike", "ground" ]

        self.im_idx = []
        for i_folder in split:
            files = sorted(glob.glob(os.path.join(self.data_path, str(i_folder).zfill(2), 'velodyne', '*.bin')))
            self.im_idx.extend(files)
    ...
    def __getitem__(self, index):
        ...
        inslabels = read_inslabels(self.im_idx[index].replace('velodyne', 'labels')[:-3] + 'label')
        inslabels = inslabels.reshape(-1, 1)
        ...
        inslabels = inslabels[mask]

        grid_shape_tmp = [256, 256, 32]
        intervals_tmp = (self.max_volume_space - self.min_volume_space) / (grid_shape_tmp)
        grid_index_tmp = (np.floor((xyz - self.min_volume_space) / intervals_tmp)).astype(np.int32)
        ...
        if self.dataset_type == 'test':
            ...
            voxel_label_path = os.path.join(self.label_path, sequence, "voxels", frame_id + ".label")
            voxel_invalid_path = os.path.join(self.label_path, sequence, "voxels", frame_id + ".invalid")
            voxel_label_1_1 = _read_label_SemKITTI(voxel_label_path).astype(np.uint16)
            voxel_label_1_1 = self.remap_lut[voxel_label_1_1].astype(np.float32)
            voxel_invalid = _read_invalid_SemKITTI(voxel_invalid_path)
            voxel_label_1_1[np.isclose(voxel_invalid, 1)] = 255
            voxel_label_1_1 = voxel_label_1_1.reshape(256, 256, 32)
