...
class SSCMetrics:
    ...
    def add_batch(self, y_pred, y_true, nonempty=None, nonsurface=None):
        '''
        y_pred: (bs, H, W, D) <-- np.argmax((bs, n_classes, H, W, D), axis=1)
        y_true: (bs, H, W, D)
        '''
        self.count += 1
        mask = y_true != 255
    ...
    def get_score_completion(self, predict, target, nonempty=None):
        '''
        predict: (bs, H, W, D) <-- np.argmax((bs, n_classes, H, W, D), axis=1)
        target: (bs, H, W, D)
        '''
        ...
        target[target == 255] = 0
        # ---- flatten
        target = target.reshape(_bs, -1)    # (bs, H*W*D)
        predict = predict.reshape(_bs, -1)  # (bs, H*W*D)
        # ---- treat all non-empty object class as one category, set them to label 1
        b_pred = np.zeros(predict.shape)    # (bs, H*W*D)
        b_true = np.zeros(target.shape)     # (bs, H*W*D)
        b_pred[predict > 0] = 1             # pred: non-empty
        b_true[target > 0] = 1              # target: non-empty 
        p, r, iou = 0.0, 0.0, 0.0
        tp_sum, fp_sum, fn_sum = 0, 0, 0
        for idx in range(_bs):
            y_true = b_true[idx, :]         # (H*W*D)
            y_pred = b_pred[idx, :]         # (H*W*D)
            if nonempty is not None:
    ...
    def get_score_semantic_and_completion(self, predict, target, nonempty=None):
        ...
        for idx in range(_bs):
            y_true = target[idx, :]         # (H*W*D)
            y_pred = predict[idx, :]        # (H*W*D)
            if nonempty is not None:
                nonempty_idx = nonempty[idx, :].reshape(-1)
                y_pred = y_pred[
        ...
        return tp_sum, fp_sum, fn_sum   # (n_classes), (n_classes), (n_classes)
    



## just for scene completion
class SceneCompletionMetrics:
    def __init__(self, n_classes):
        self.n_classes = n_classes
        self.reset()
