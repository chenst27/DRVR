...
from models.CENet import ResNet_34, convert_relu_to_softplus, load_partial_weights, load_partial_weights_v2
...
class DRVR(nn.Module):
    def __init__(self, config):
        ...
        self.sparse_embed_dims = 128
        self.fine_sparse_decoder_1_2 = SparseVFE(in_channel=self.sparse_embed_dims, out_channel=self.sparse_embed_dims, indice_key='fine_sparse_decoder_1_2')
        self.fine_classifier_1_1 = spconv.SparseSequential(
            spconv.SubMConv3d(self.sparse_embed_dims, self.sparse_embed_dims, indice_key="fine_classifier_1_1_1", kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm1d(self.sparse_embed_dims),
            nn.ReLU(inplace=True),
            spconv.SubMConv3d(self.sparse_embed_dims, self.num_classes, indice_key="fine_classifier_1_1_2", kernel_size=3, stride=1, padding=1, bias=True)
        )
    ...
    def forward(self, data_dict):
        ...
        # sparse_feats = spconv.SparseConvTensor(sparse_feats_1_2, coors_unq_1_2, [128,128,16], 1)
