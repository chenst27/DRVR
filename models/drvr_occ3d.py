...
class DRVR(nn.Module):
    def __init__(self, config):
        ...
        self.fine_classifier_1_1 = spconv.SparseSequential(
            spconv.SubMConv3d(self.embed_dims, 64, indice_key="fine_classifier_1_1_1", kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            spconv.SubMConv3d(64, self.num_classes, indice_key="fine_classifier_1_1_2", kernel_size=3, stride=1, padding=1, bias=True)
        )
    ...
    def forward(self, data_dict):
        ...
        sc_logits_1_2, bev_geo_feats = self.bev_encoder(data_dict)
        sc_logits_1_2_tmp = sc_logits_1_2.clone().permute(0, 2, 3, 4, 1).flatten(0, 3).contiguous()
        ...
        # forward projection
        vox_feats_1_2_all = torch.zeros(((self.bev_h) * (self.bev_w) * (self.bev_z), self.embed_dims), device=rv_feats.device)
        ...
        vox_feats_1_2_all = vox_feats_1_2_all.reshape(1, self.bev_h, self.bev_w, self.bev_z, -1).permute(0, 4, 1, 2, 3).contiguous()
        ...
        point_feats = spconv.SparseConvTensor(point_feats, point_coors_unq, [100,100,8], 1)
        ...
        sparse_geo_feats = spconv.SparseConvTensor(geo_feats, coors_unq_1_2, [100,100,8], 1)
        ...
        sparse_fused_feats = spconv.SparseConvTensor(fused_feats, coors_unq_1_2, [100,100,8], 1)
        sparse_fused_feats = self.fusion_layer(sparse_fused_feats)

        dense_fine_feats_1_2 = sparse_fused_feats.dense()
        dense_fine_feats_1_2 = dense_fine_feats_1_2.flatten(2).permute(0, 2, 1).contiguous()
        dense_fine_feats_1_1 = dense_fine_feats_1_2[:, data_dict['f2cmapping_1_2'][0], :].contiguous()

        dense_fine_feats_1_1 = dense_fine_feats_1_1.reshape(1, 200, 200, 16, -1).permute(0, 4, 1, 2, 3).contiguous()
        coors_unq_1_1, sparse_feats_1_1 = extract_nonzero_features(dense_fine_feats_1_1)
        sparse_feats_tensor_1_1 = spconv.SparseConvTensor(sparse_feats_1_1, coors_unq_1_1, [200,200,16], 1)
        fine_logits_1_1 = self.fine_classifier_1_1(sparse_feats_tensor_1_1)
        ssc_logits_1_1 = fine_logits_1_1.dense()

        # get mask of predictions
        dense_fine_logits_1_1 = fine_logits_1_1.dense().clone()
        dense_fine_logits_1_1 = dense_fine_logits_1_1.flatten(2).permute(0, 2, 1).contiguous()
        nonzero_index_1_1 = torch.sum(torch.abs(dense_fine_logits_1_1[0]), dim=1) != 0

        preds = {}
        preds['ssc_logits_1_1'] = ssc_logits_1_1                # (bs, n_class, 200, 200, 16)
        preds['sc_logits_1_2'] = sc_logits_1_2                  # (bs, n_class, 100, 100, 8)
        preds['num_nonempty_1_1'] = nonzero_index_1_1.sum()
        preds['num_nonempty_1_2'] = nonempty_mask_1_2.sum()
        preds['pred_mask_1_1'] = nonzero_index_1_1              # (200*200*16, )
        return preds
