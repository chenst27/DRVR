...
from models.drvr_smp import DRVR
from dataloader.dataset_semposs_ssc import SemanticPOSS, collate_fn_default
...
class Trainer():
    ...
    def _initDataloader(self):
        train_dataset = SemanticPOSS(config=self.config, dataset_type='train', flip_aug=self.config['dataset_params']['train_data_loader']['flip_aug'])
        valid_dataset = SemanticPOSS(config=self.config, dataset_type='valid', flip_aug=self.config['dataset_params']['val_data_loader']['flip_aug'])
    ...
    def train(self, epoch):
        ...
        for i, data_dict in enumerate(self.train_loader):
            ...
            # voxel_label_1_2
            B = voxel_label_1_1.shape[0]
            voxel_label_1_2 = voxel_label_1_1.reshape(B, 128, 2, 128, 2, 16, 2).permute(0,1,3,5,2,4,6).reshape(B, 128, 128, 16, 8)
            ...
            num_voxel_label_1_2_0 = len(occ_space_1_2[occ_space_1_2==0])
            num_voxel_label_1_2_255 = len(occ_space_1_2[occ_space_1_2==255])
            occ_space_1_2[occ_space_1_2==255] = torch.arange(300, 300 + num_voxel_label_1_2_255).to(occ_space_1_2.device)
            occ_space_1_2[occ_space_1_2==0] = torch.arange(400 + num_voxel_label_1_2_255, 400 + num_voxel_label_1_2_255 + num_voxel_label_1_2_0).to(occ_space_1_2.device)
            voxel_label_1_2[~empty_mask_1_2] = occ_space_1_2
            voxel_label_1_2 = torch.mode(voxel_label_1_2, dim=-1)[0]
            voxel_label_1_2[voxel_label_1_2 > (390 + num_voxel_label_1_2_255)] = 0
            voxel_label_1_2[voxel_label_1_2 > 290] = 255
    ...
    def valid(self, epoch):
        ...
        loss_meter = AverageMeter()
        loss_geo_focal_1_2_meter = AverageMeter()
        loss_geo_dice_1_2_meter = AverageMeter()
        loss_geo_focal_1_1_meter = AverageMeter()
        loss_geo_dice_1_1_meter = AverageMeter()
        loss_sem_focal_1_1_meter = AverageMeter()
        loss_sem_dice_1_1_meter = AverageMeter()
        ...
        with torch.no_grad():
            for i, data_dict in enumerate(self.val_loader):
                ...
                loss_geo_focal_1_2_meter.update(loss_geo_focal_1_2.item())
                loss_geo_dice_1_2_meter.update(loss_geo_dice_1_2.item())
                loss_geo_focal_1_1_meter.update(loss_geo_focal_1_1.item())
                loss_geo_dice_1_1_meter.update(loss_geo_dice_1_1.item())
                loss_sem_focal_1_1_meter.update(loss_sem_focal_1_1.item())
                loss_sem_dice_1_1_meter.update(loss_sem_dice_1_1.item())
            ...
            if self.recorder is not None:
                ...
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/geo_focal_1_2", scalar_value=loss_geo_focal_1_2_meter.avg, global_step=epoch)
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/geo_dice_1_2", scalar_value=loss_geo_dice_1_2_meter.avg, global_step=epoch)
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/geo_focal_1_1", scalar_value=loss_geo_focal_1_1_meter.avg, global_step=epoch)
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/geo_dice_1_1", scalar_value=loss_geo_dice_1_1_meter.avg, global_step=epoch)
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/sem_focal_1_1", scalar_value=loss_sem_focal_1_1_meter.avg, global_step=epoch)
                self.recorder.tensorboard.add_scalar(
                    tag="Valid_Loss_Details/sem_dice_1_1", scalar_value=loss_sem_dice_1_1_meter.avg, global_step=epoch)
