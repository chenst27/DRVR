...
valid_infos = valid_dataset.data_info
...
for i, data_dict in enumerate(valid_loader):
    with torch.no_grad():
        ...
        if save_pred:
            scene_index = valid_infos[i]['scene_index']
            sample_token = valid_infos[i]['sample_token']
            folder_dir = os.path.join(pred_dir, scene_index, sample_token)
            os.makedirs(folder_dir, exist_ok=True)
            pred_path = os.path.join(folder_dir, 'labels.npz')
            to_save = ssc_logits_eval.clone().cpu().numpy().astype(np.uint8)
            mask_0 = (to_save == 0)
            mask_17 = (to_save == 17)
            to_save[mask_0] = 17
            to_save[mask_17] = 0
            np.savez_compressed(pred_path, semantics=to_save.reshape(200, 200, 16))
