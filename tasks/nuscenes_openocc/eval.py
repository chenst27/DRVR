...
valid_dataset = NuscenesOpenOccupancy(
    config=config, 
    pkl_file=config['infer_params']['pkl_file']
    )
cls_name = valid_dataset.class_names
valid_infos = valid_dataset.nusc_infos
...
for i, data_dict in enumerate(valid_loader):
    with torch.no_grad():
        ...
        if save_pred:
            info = valid_infos[i]
            folder_name = 'scene_{}/occupancy'.format(info['scene_token'])
            folder_dir = os.path.join(pred_dir, folder_name)
            os.makedirs(folder_dir, exist_ok=True)
            file_name = '{}.npy'.format(info['lidar_token'])
            pred_path = os.path.join(folder_dir, file_name)
            ssc_logits_np = ssc_logits_eval.cpu().numpy()
            ssc_logits_np = ssc_logits_np.astype(np.int32).reshape((512, 512, 40))
            np.save(pred_path, ssc_logits_np)
